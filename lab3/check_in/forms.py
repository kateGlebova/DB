...
from check_in.models import Room, Client


def get_room_choice_list():
    return list((room.id, room.id) for room in Room.objects.all())


def get_client_choice_list():
    return list((client.id, client.id) for client in Client.objects.all())
